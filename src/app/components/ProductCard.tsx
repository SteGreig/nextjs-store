...
export const ProductCard = ({product} : Props) => (
  <article key={product.id} className="flex flex-col bg-white shadow-xl hover:shadow-2xl transition duration-300 rounded-xl overflow-hidden">
    <div className='p-2 pb-0'>
    <div className='w-full aspect-square relative bg-gray-100 rounded-lg'>
      <Image className='w-full h-full object-contain' width={200} height={200} src={product.images[0]} alt={product.title} />
    </div>
    </div>
    <div className='p-4 flex flex-col flex-1 gap-2'>
      <h3 className='font-semibold leading-snug'>{product.title}</h3>
      <p className="text-gray-500 font-light text-sm/snug hidden md:block">{truncate(product.description, 90)}...</p>
    </div>
